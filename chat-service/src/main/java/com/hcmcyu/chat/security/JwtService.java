...
public class JwtService {
    ...
    public CurrentUser parseAccessToken(String token) {
        try {
            ...
            throw new ChatServiceException(HttpStatus.UNAUTHORIZED, "TOKEN_EXPIRED", "Access token has expired");
        } catch (IllegalArgumentException | JwtException | NullPointerException exception) {
            throw new ChatServiceException(HttpStatus.UNAUTHORIZED, "INVALID_TOKEN", "Access token is invalid");
        }
    }
}
